...
let PacInitialize: any;
let PacShutdown: any;
let PacSetLEDState: any;
let PacSetLEDStates: any;
let PacGetVendorId: any;
let PacGetProductId: any;
let PacGetDeviceType: any;
let dll: any;
...
if (!fs.existsSync(dllPath)) {
  ...
  try {
    ...
    dllLoadError = `Failed to load PacDrive DLL: ${error instanceof Error ? error.message : String(error)}`;
 ...
 */
export function getDeviceInfo(deviceId: number): { vendorId: string; productId: string; } | null {
  if (!initialized || !dllLoaded) {
 ...
 */
export function testDeviceConnection(deviceId: number): boolean {
  if (!dllLoaded) {
...
export function initialize(): boolean {
  try {
    ...
    if (!dllLoaded) {
      initializationError = `Cannot initialize PacDrive: DLL not loaded: ${dllLoadError}`;
    ...
    if (result !== 0) {
      ...
      initializationError = `PacDrive initialization failed with code: ${result}`;
      logger.error(initializationError);
      return false;
    }
  } catch (error) {
    initializationError = `Error initializing PacDrive: ${error instanceof Error ? error.message : String(error)}`;
    logger.error(initializationError);
    return false;
  }
}
 ...
 * @returns {boolean} true if successful, false otherwise
 */
export function setOutput(deviceId: number, outputIndex: number, state: number): boolean {
  try {
 ...
 * @returns {boolean} true if successful, false otherwise
 */
export function setOutputs(deviceId: number, bitmask: number): boolean {
  try {
    // Check if DLL is loaded first
    if (!dllLoaded) {
 ...
 */
export function getDllDiagnostics(): {
  dllPath: string;
  dllExists: boolean;
  dllLoaded: boolean;
  dllLoadError: string | null;
...
export const pacDriveController = {
  initialize,
  shutdown,
  setOutput,
  setOutputs,
...
export default pacDriveController;
